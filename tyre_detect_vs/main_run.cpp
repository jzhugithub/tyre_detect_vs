...
int main()  
{
	...
	//---------------------------------------------------------------
	//变量定义
	HOGDescriptor detectHOG(WinSizeDetect,BlockSizeDetect,BlockStrideDetect,CellSizeDetect,NbinsDetect,1,-1,0,0.2,false,15);//分类HOG检测器
   ...
   for(int i=0; i<supportVectorDetectNum; i++)//将alpha向量的数据复制到alphaMat中  
	...
	for (int fnum = 1;!stop;fnum++)
	{
		...
		//对图片进行多尺度机器人检测 
		cout<<"进行多尺度检测"<<endl;
		detectHOG.detectMultiScale(src, found, HitThreshold, WinStride, Size(0,0), DetScale, 0, true);
